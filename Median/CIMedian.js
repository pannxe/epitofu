...
function CalcCIMedian(data) {
  ...
  var p = 0;
  var p = median / 100;
  if (p == 0.5) var np = (n + 1) / 2;
  else var np = Math.round(n * p);

  //confidence limits and checking errors;
  var ll = Math.round(n * p - z * Math.sqrt(n * p * (1 - p)));
  if (ll <= 0) var ll = 0;
  var ul = Math.round(1 + n * p + z * Math.sqrt(n * p * (1 - p)));
  if (ul <= 0) var ul = 0;
  if (ul >= n) var ul = n;
  ...
  with (outTable) {
    ...
    newrow(
      "",
      "color#66ffff:span2:r:Desired percentile:",
      "color#ffff99:c:" + median
    );
    newrow(
      "",
      "color#66ffff:span2:r:Confidence Interval (%):",
      "color#ffff99:c:" + pt
    );
    newrow();
    ...
    newrow(
      "span2:c:Normal Approximation",
      "c:" + fmtSigFig(ll, 6),
      "r:" + fmtSigFig(np, 6),
      "span2:c:" + fmtSigFig(ul, 6)
    ); //6 means 6 digits including decimals;

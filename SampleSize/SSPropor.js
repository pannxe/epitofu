...
var Description =
  "This module calculates sample size for determining " +
...
function Calc(data) {
  function ssx(conflevel) {
    ...
    Za2 = ANorm(Za1);
    sampsize =
      (deffx * N_totpop * proportionx * (1 - proportionx)) /
      (((d_precisionx * d_precisionx) / (Za2 * Za2)) * (N_totpop - 1) +
        proportionx * (1 - proportionx));
    return Math.ceil(sampsize);
  }

  var formula =
    "<b><i>n</i> = [DEFF*Np(1-p)]/ [(d<sup>2</sup>/Z<sup>2</sup><sub>1-<span>&#945;/2</span></sub>*(N-1)+p*(1-p)]&nbsp;&nbsp;</B>";

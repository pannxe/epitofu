...
var Authors =
  "<b>Statistics</b><br>Kevin M. Sullivan, Emory University<br>" +
  "based on code from John C. Pezzullo<br>" +
  "<b>Interface</b><br>" +
  "Andrew G. Dean, EpiInformatics.com, and Roger A. Mir<br> ";

var Description =
  "This module calculates sample size for an unmatched " +
...
function Calc(data) {
  ...
  var qbar = 1 - pbar;
  var vn =
    (Math.pow(Za + Zb, 2) * pbar * qbar * (vr + 1)) /
    (Math.pow(v1 - v2, 2) * vr);
  var vn1 =
    Math.pow(
      Za * Math.sqrt((vr + 1) * pbar * qbar) +
        Zb * Math.sqrt(vr * v1 * (1 - v1) + v2 * (1 - v2)),
      2
    ) /
    (vr * Math.pow(v2 - v1, 2));
  vn2 =
    ...
    (vr * Math.pow(Math.abs(v1 - v2), 2));
  vn2 =
    (vn2 *
      Math.pow(
        1 + Math.sqrt(1 + (2 * (vr + 1)) / (vn2 * vr * Math.abs(v2 - v1))),
        2
      )) /
    4;
  ...
  with (outTable) {
    ...
    newrow(
      "",
      "span3:Two-sided confidence level(1-alpha)",
      parseFloat(data[1]["E0D0"])
    );
    newrow(
      "",
      "span3:Power(% chance of detecting)",
      parseFloat(data[1]["E1D0"])
    );
    newrow("", "span3:Ratio of Controls to Cases", parseFloat(data[1]["E2D0"]));
    newrow(
      "",
      "span3:Hypothetical proportion of controls with exposure",
      parseFloat(data[1]["E3D0"])
    );
    newrow(
      "",
      "span3:Hypothetical proportion of cases with exposure::",
      parseFloat(data[1]["E6D0"]),
      ""
    );
    newrow(
      "",
      "span3:Least extreme Odds Ratio to be detected::",
      vor.toFixed(2)
    );
    newrow();
    newrow("span2:", "bold:c:Kelsey", "bold:c:Fleiss", "bold:c:Fleiss with CC");
    newrow(
      "span2:Sample Size - Cases",
      Math.ceil(vn),
      Math.ceil(vn1),
      Math.ceil(vn2)
    );
    newrow(
      "span2:Sample Size - Controls",
      Math.ceil(vn * vr),
      Math.ceil(vn1 * vr),
      Math.ceil(vr * vn2)
    );
    // line(6);
    newrow(
      "span2:Total sample size::",
      Math.ceil(vn) + Math.ceil(vn * vr),
      Math.ceil(vn1) + Math.ceil(vn1 * vr),
      Math.ceil(vr * vn2) + Math.ceil(vn2)
    );
    newrow();
    newrow("span6:c:color#c0c0c0:References");
    newrow(
      "span6:Kelsey et al., Methods in Observational Epidemiology 2nd Edition, Table 12-15"
    );
    newrow(
      "span6:Fleiss, Statistical Methods for Rates and Proportions, formulas 3.18 &amp;3.19"
    );
    newrow();
    newrow("span6:CC = continuity correction");
    newrow("span6:Results are rounded up to the nearest integer.");
    newrow(
      "span6:Print from the browser menu or select, copy, and paste to other programs."
    );
  }
}

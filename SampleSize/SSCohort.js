...
var Description =
  "This module calculates sample size for unmatched " +
...
function Calc(data) {
  ...
  with (outTable) {
    ...
    title(Title);
    newrow(
      "span4:Two-sided significance level(1-alpha)::",
      parseFloat(data[1]["E0D0"])
    );
    newrow(
      "span4:Power(1-beta, % chance of detecting)::",
      parseFloat(data[1]["E1D0"])
    );
    newrow(
      "span4:Ratio of sample size, Unexposed/Exposed::",
      parseFloat(data[1]["E2D0"])
    );
    newrow("span4:Percent of Unexposed with Outcome::", fmtSigFig(v2, 2, 100));
    newrow(
      "span4:Percent of Exposed with Outcome::",
      fmtSigFig(v1, 2, 100),
      ""
    );
    ...
    line(6);
    newrow(
      "span2:Sample Size - Exposed",
      Math.ceil(vn),
      Math.ceil(vn1),
      Math.ceil(vn2)
    );
    newrow(
      "span2:Sample Size-Nonexposed",
      Math.ceil(vn * vr),
      Math.ceil(vn1 * vr),
      Math.ceil(vr * vn2)
    );

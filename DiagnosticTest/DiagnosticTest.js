...
var Description =
  "<p>This module helps evaluate the performance of a candidate diagnostic test or screening procedure against a known <i>reference</i> ";
Description +=
  "or <i>gold</i> standard. It is assumed that a reference standard (the true diagnosis) exists. The module lets you choose the type of diagnostic test ";
Description +=
  "based on the outcome it reports. Then, the module uses pertinent statistics to evaluate the performance of the diagnostic test.</p> ";
Description +=
  "<p>For two or more exposure levels, the program calculates Sensitivity, Specificity, Positive and Negative Predictive Values, ";
Description +=
  "Diagnostic Accuracy, Likelihood Ratios, Diagnostic Odds, Cohen's Kappa, Entropy Reduction, and a Bias Index.</p> ";
Description +=
  "<p>If more than two levels of results are specified, a plot (ROC curve) showing proportions of true positives versus false ";
...
var Demo =
  "Run Diagnostic Test Evaluation and click on Load Demo Data  " +
  "<ul>" +
...
var Exercises =
  "A rapid screening test is compared with definitive laboratory testing of the same persons as the gold standard' " +
  "The results are: " +
  "<ul>" +
...
function assesstwobytwo() {
  ...
  with (outObj) {
    newrow(
      ...
      "",
      "c:bold:Estimate",
      "c:span2:bold:Lower - Upper\n95% CIs",
      "c:span2:bold:Method"
    );
    line(6);
    newrow(
      "span2:Sensitivity:",
      "c:" + fmtSigFig(sn, 4, 100) + "%",
      "c:span2:(" + limits(snl, snu, 1, 100) + ")",
      "span2:Wilson Score"
    );
    newrow(
      "span2:Specificity:",
      "c:" + fmtSigFig(sp, 4, 100) + "%",
      "c:span2:(" + limits(spl, spu, 1, 100) + ")",
      "span2:Wilson Score"
    );
    newrow(
      "span2:Positive Predictive Value:",
      "c:" + fmtSigFig(ppv, 4, 100) + "%",
      "c:span2:(" + limits(ppvl, ppvu, 1, 100) + ")",
      "span2:Wilson Score"
    );
    newrow(
      "span2:Negative Predictive Value:",
      "c:" + fmtSigFig(npv, 4, 100) + "%",
      "c:span2:(" + limits(npvl, npvu, 1, 100) + ")",
      "span2:Wilson Score"
    );
    newrow(
      "span2:Diagnostic Accuracy:",
      "c:" + fmtSigFig(acc, 4, 100) + "%",
      "c:span2:(" + limits(accl, accu, 1, 100) + ")",
      "span2:Wilson Score"
    );

    newrow(
      "span2:Likelihood ratio of a Positive Test:",
      "c:" + fmtSigFig4(lrp),
      "c:span2:(" + fmtSigFig4(lrpl) + " - " + fmtSigFig4(lrpu) + ")"
    );
    newrow(
      "span2:Likelihood ratio of a Negative Test:",
      "c:" + fmtSigFig4(lrn),
      "c:span2:(" + fmtSigFig4(lrnl) + " - " + fmtSigFig4(lrnu) + ")"
    );
    newrow(
      "span2:Diagnostic Odds:",
      "c:" + fmtSigFig4(odds),
      "c:span2:(" + fmtSigFig4(odl) + " - " + fmtSigFig4(odu) + ")"
    );
    newrow(
      "span2:Cohen's kappa (Unweighted):",
      "c:" + fmtSigFig4(kap),
      "c:span2:(" + fmtSigFig4(kapl) + " - " + fmtSigFig4(kapu) + ")"
    );
    newrow(
      "span2:Entropy reduction after a Positive Test:",
      "c:" + fmtSigFig(entp, 4, 100) + "%"
    );
    newrow(
      "span2:Entropy reduction after a Negative Test:",
      "c:" + fmtSigFig(entn, 4, 100) + "%"
    );
  ...
  for (lc = 1; lc < levels + 1; lc++) {
    ...
    with (outObj) {
      newrow(
        ...
        "Level " + lc,
        fmtSigFig4(lr),
        "span2:" + fmtSigFig4(lrl) + " - " + fmtSigFig4(lru)
      );
    }
  }
  with (outObj) {
...
function calcauc() {
  ...
  for (ac = 1; ac < levels + 1; ac++) {
    auc5[ac - 1] =
      auc1[ac - 1] * auc2[ac - 1] + 0.5 * auc1[ac - 1] * auc3[ac - 1];
    auc6[ac - 1] =
      auc3[ac - 1] *
      (auc4[ac - 1] * auc4[ac - 1] +
        auc4[ac - 1] * auc1[ac - 1] +
        (auc1[ac - 1] * auc1[ac - 1]) / 3);
    auc7[ac - 1] =
      auc1[ac - 1] *
      (auc2[ac - 1] * auc2[ac - 1] +
        auc2[ac - 1] * auc3[ac - 1] +
        (auc3[ac - 1] * auc3[ac - 1]) / 3);

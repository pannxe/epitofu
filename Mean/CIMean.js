...
function CalcCIMean(data) {
  ...
  if (
    ...
    alert(
      "Select only one! - Standard deviation or Standard error or Variance??"
    );
    return false;
  }
  ...
  if (popsize.length < 1 || popsize.length > 9 || popsize < n) {
    alert(
      "Population size must be between 1 and 999,999,999  &  must be >= sample size"
    );
    return false;
  }
  ...
  if (pt == 20) cscrit = 0.064;
    ...
    alert(
      "The selected confidence interval is not available, choose other ranges"
    );
    return false;
  }
  z = Math.sqrt(cscrit);
  ...
  //for z distribution;
  var ll =
    mean - z * (SD / Math.sqrt(n)) * Math.sqrt((popsize - n) / (popsize - 1));
  var ul =
    mean + z * (SD / Math.sqrt(n)) * Math.sqrt((popsize - n) / (popsize - 1));
  ...
  var t = AStudT(p, n - 1);
  var llt =
    mean - t * (SD / Math.sqrt(n)) * Math.sqrt((popsize - n) / (popsize - 1));
  var ult =
    mean + t * (SD / Math.sqrt(n)) * Math.sqrt((popsize - n) / (popsize - 1));
  ...
  with (outTable) {
    ...
    newrow(
      "color#66ffff:span3:bold:c:Sample Mean",
      "color#ffff99:span5:l:" + fmtSigFig(mean, 6)
    );
    newrow(
      "color#66ffff:span3:bold:c:Sample Std. Deviation",
      "color#ffff99:span1:l:" + fmtSigFig(SD, 6),
      "color#ffff99:span1:bold:l:Std. Error",
      "color#ffff99:span1:l:" + fmtSigFig(SE, 6),
      "color#ffff99:span1:bold:l:Variance",
      "color#ffff99:span1:l:" + fmtSigFig(variance, 6)
    );
    newrow(
      "color#66ffff:span3:bold:c:Sample size",
      "color#ffff99:span5:l:" + fmtSigFig(n, 6)
    );
    newrow(
      "color#66ffff:span3:bold:c:Population size",
      "color#ffff99:span5:l:" + fmtSigFig(popsize, 9)
    );
    newrow(
      "color#66ffff:span3:bold:c:Confidence Interval",
      "color#ffff99:span1:l:" + fmtSigFig(pt, 6),
      "color#ffff99:span4:bold:l:% (100)"
    );
    line(8);
    ...
    title(
      "<h4>  " +
        pt +
        "% Confidence Limits for the Mean of " +
        parseFloat(data[1]["E0D0"]) +
        "</h4>"
    );
    newrow(
      "span4:bold:c:Based on:: ",
      "span2:bold:c:Lower Limit",
      "span2:c:bold:Upper Limit"
    );
    newrow(
      "span4:c: z-test",
      "span2:c:" + fmtSigFig(ll, 6),
      "span2:c:" + fmtSigFig(ul, 6)
    );
    newrow(
      "span4:c: t-test",
      "span2:c:" + fmtSigFig(llt, 6),
      "span2:c:" + fmtSigFig(ult, 6)
    );
    line(8);
    endtable();
  }

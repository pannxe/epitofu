...
var Description =
  "Use this table to test for an association between variables with more than 2 " +
...
function CalcStats(cmdObj) {
  ...
  with (cmdObj) {
    ...
    with (cmdObj) {
      ...
      if (expunder5percent == 0) {
        expunder5percent = "None";
      } else {
        expunder5percent += "%";
      }
      newrow(
      ...
      newrow();
      if (expunder5 / (numE * numD) > 0.2 || expunder1 > 0) {
        newrow("", "span5:This table DOES NOT MEET Cochran's criteria.");
      } else {
        newrow(
          "",
          "span5:Using these criteria, this chi square can be accepted."
        );
      }
      newrow();
      ...
      newrow(
        "",
        "span6:Rosner, B. Fundamentals of Biostatistics. 5th ed. Duxbury Thompson Learning. 2000; p. 395"
      );
      endtable();
    }
  }

function Stratum(x, y, w, h, r, c, name) {
  ...
  if (name) {
    this.name = name + Stratum.count++;
  } else {
    this.name = "Stratum" + Stratum.count++;
  }
...
function StratumCellWrite(r, c) {
  ...
  this.row[r].cell[c].dynlayer.write(
    "<div class='" +
      this.name +
      "innerCell'><table border=0 width=" +
      this.cellwidth * span +
      " height=" +
      this.cellheight +
      " cellpadding=" +
...
function StratumSetRow() {
  ...
  for (var i = 0; i < arguments.length - 1; i = i + 2) {
    if (this.lockCheck) {
      if (this.lockCheck(r, this.cellCount)) {
        return true;
      }
    }
    this.row[r].cell[this.cellCount].value = arguments[i + 1];
    if (arguments[i + 2])
      this.row[r].cell[this.cellCount].href = arguments[i + 2];
    this.cellCount++;
  }
}
...
function StratumEmptyCellsinRow(row) {
  ...
  for (col = 0; col < this.cols; col++) {
    if (
      this.row[row].cell[col].value.length == 0 &&
...
function StratumEmptyCellsinCol(c) {
  ...
  for (r = 0; r < this.rows; r++) {
    if (
      this.row[r].cell[c].value.length == 0 &&
      this.row[r].cell[c].type == "data"
    ) {
      count += 1;
    }
...
var singleempc = t(
  "There must be a single empty cell in column to enter total."
);
...
function addCalendar() {
   ...
   */
    ...
    addCalendar.currentCalendar.bind("change", function () {
...
function StratumMoveInputTo(r, c) {
  ...
  if (enter) {
    ...
    var newcss = css(
      ...
      this.Hpos,
      this.Vpos,
      this.cellwidth * this.row[r].cell[c].span,
      this.cellheight,
      "#ff0000",
      "visible",
      1
    ...
    this.inputbox.moveTo(
      this.row[r].cell[c].dynlayer.x,
      this.row[r].cell[c].dynlayer.y
    );
    ...
    */
    ...
    if (!bcolor) {
      bcolor = this.cellColor;
    }
    var nwidth =
      this.row[r].cell[c].span * this.cellwidth +
      (this.row[r].cell[c].span - 1) * this.spacing;
    if (nwidth < 0) {
      nwidth = 0;
    } //added Mar 2007
...
function StratumMoveInputNext() {
  ...
  do {
    ...
    this.row[row].cell[col].type != "data" &&
    this.row[row].cell[col].type != "calendar" &&
    tries <= this.rows * this.cols
  );
...
function StratumBuild() {
  ...
  this.celltotal = this.cellwidth * this.cols + this.spacing * (this.cols - 1);

  //determine cell heights based on table height, account for header
  if (this.header) {
    this.totalrows = this.rows + 1;
  } else {
  ...
  this.rowtotal =
    this.cellheight * this.totalrows + this.spacing * (this.totalrows - 1);
  ...
  this.css += css(
    this.name + "ShadowDiv",
    this.x + this.shadowwidth,
    this.y + this.shadowwidth,
    this.w,
    this.h,
    this.shadowcolor,
    "hidden",
...
*/
  ...
  this.css +=
    "#InputDiv INPUT {font-family : Trebuchet MS;font-size : 10pt;font-weight : bold; background-color:#ddddff; border-color : #336699;" +
  ...
  for (i = 0; i < this.row.length; i++) {
    ...
    for (j = 0; j < this.row[i].cell.length; j++) {
      ...
      this.css += css(
        ...
        this.cellColor,
        "visible"
      ); //Mar 2007 show changed to visible
...
function StratumActivate() {
  ...
  for (var i = 0; i < this.row.length; i++) {
    for (var j = 0; j < this.row[i].cell.length; j++) {
      this.row[i].cell[j].dynlayer = new DynLayer(
        this.name + "Row" + i + "Cell" + j + "Div"
      );
      ...
      if (is.ns4) {
        this.row[i].cell[j].dynlayer.event.captureEvents(Event.MOUSEDOWN);
        this.row[i].cell[j].dynlayer.event.onmousedown = new Function(
          this.obj + ".down(" + i + "," + j + "); return false;"
        );
      }
      this.row[i].cell[j].dynlayer.event.onmousedown = new Function(
        this.obj + ".down(" + i + "," + j + "); return false;"
      );
      this.row[i].cell[j].dynlayer.event.onmouseover = new Function(
        this.obj + ".over(" + i + "," + j + "); return false;"
      );
      this.row[i].cell[j].dynlayer.event.onmouseout = new Function(
        this.obj + ".out(" + i + "," + j + "); return false;"
      );
    }
  }

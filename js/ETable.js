...
function setColors() {
  ...
  for (r = 0; r < myTable.rows; r++) {
    for (c = 0; c < myTable.cols; c++) {
      ...
      // alert("r="+r+" c="+"tt="+tt)
      if (tt == "data" || tt == "calendar") {
        ccolor = datacolor;
      } else if (tt == "rowtot" || tt == "row total" || tt == "column total") {
        ccolor = totalcolor;
      } else if (tt == "valname") {
        ccolor = valuecolor;
      } else if (tt == "varname") {
        ccolor = variablecolor;
      } else if (tt == "label") {
        ccolor = variablecolor;
      }
...
function addStratum() {
  ...
  if (
    ...
    var option = new Option(
      "Stratum " + (strat.options.length + 1),
      "Optional Name",
      false,
      true
    );
...
function timerStop(hideAfter) {
  ...
  try {
    ...
    document.getElementById("proc_span").innerHTML =
      "Done<br>" + elapsedtime + " seconds";
  } catch (err) {
    //ignore error
  }
  setTimeout(
...
function summaryTable() {
  ...
  htmlstr +=
    '<button type="button" onClick="clearTable();return false;">' +
    clearStr +
    "</button>";

  htmlstr +=
    '<span id="settings_span" name="settings_span" style="visibility:hidden;">';

  htmlstr +=
    ' <a name="settinglink" href="../settings/Settings.htm" target="_self">' +
    "Settings " +
    "</a>";

  //htmlstr+='<font size=-1>&nbsp;Conf. level &nbsp;\='+ConfLevel+'\%</font>';
  htmlstr +=
    '<small style="text-align:left">&nbsp;Conf. level=' +
    ConfLevel +
    "%</small>";
  ...
  htmlstr +=
    '<button type="button" id="calc" onClick="callStatsIf();return false;">' +
    calculateStr +
    "</button>";
  ...
  htmlstr += "<td>";
  htmlstr +=
    '<INPUT TYPE="button" VALUE="' + t("Add Stratum") + '" NAME=btnAddStrat ';
  ...
  htmlstr +=
    '<SELECT NAME=theStratum onChange="changeStratumTo(selectedIndex+1);" SIZE=1>';
  ...
  htmlstr += 'style="position:relative;top:0;left:0">';
  htmlstr +=
    '<input type="button" value="Delete Stratum" name="btnDeleteStrat" onClick="deleteStratum()">';
  htmlstr += "</span>";
  htmlstr += "</td>";
  htmlstr += "</tr>";
  htmlstr += "</table>";
...
function tableFromCommands() {
  if (typeof etablecmds1 == "object") {
    ...
    if (numcmds != -1) {
      ...
      if (etablecmds2 != null) {
        numcmds = cmdExecute(etablecmds2);
      }
    }
    document.close();
    myTable.moveInputNext();

  ...
  if (is.ns4) {
    if (is.ns4) {
      if (!frame) {
        ...
        this.css = nestref
          ? eval("frame.document." + nestref + ".document." + id)
    ...
    for (var i = 0; i < ref.layers.length; i++) {
      ...
      if (ref.layers[i].document.layers.length > 0) {
        DynLayer.refArray[DynLayer.refArray.length] =
          nestref == ""
            ? ref.layers[i].name
            : nestref + ".document." + ref.layers[i].name;
      }
    }
...
function DynLayerClipInit(clipTop, clipRight, clipBottom, clipLeft) {
  ...
  clipRight += 10;
  clipBottom += 10; //Add some pixels to allow box shadow
  //bc:if (is.ie) {
  if (is.ie || is.ns5) {
    ...
    //bc:
      ...
      this.clipTo(0, this.elm.offsetWidth + 10, this.elm.offsetHeight + 10, 0);
  }
}

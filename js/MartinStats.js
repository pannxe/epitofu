...
function martinStats(cmdObj) {
  ...
  if (this.confLevel > 1) {
    this.confLevel = this.confLevel / 100;
  }
  ...
  for (i = 0; i < this.Tables.length; i++) {
    ...
    if (curTbl.informative) {
      ...
      if (DataType == 1 || DataType == 2) {
        //                { Case-control data }
        this.minSumA += Math.round(
          Math.max(0, curTbl.m1 - curTbl.n0) * curTbl.freq
        );
        this.maxSumA += Math.round(
          Math.min(curTbl.m1, curTbl.n1) * curTbl.freq
        );
...
function Process(
  ...
  for (i = firststratum; i <= laststratum; i++) {
	 ...
	 */
      ...
      if (this.cmdObj.data[0].numD == 2 && this.cmdObj.data[0].numE == 2) {
      ...
      tbl.freq = 1; //
      tbl.m1 = tbl.a + tbl.b; // { # cases }
      tbl.informative = tbl.a * tbl.n0 != 0 || tbl.b * tbl.n1 != 0;
      /*
		alert( "stratum="+stratum+
	  "\ni="+i+
      "\na="+tbl.a+
  ...
  if (errornum != 0) {
    if (errornum == 1) {
      s = t(
        "Exact calculations skipped, since numbers are large.  Use other results."
      );
      this.cmdObj.title(s);
      this.addToArray(this.resultArray, "ERROR", 1);
    } else if (errornum == 2) {
      s = t(
        "All tables have zero marginals. Cannot perform exact calculations."
      );
      this.cmdObj.title(s);
      ...
      s = t(
        "PROBLEM: Must have only one case in each table for exact calculations."
      );
      this.cmdObj.title(s);
  ...
  if (errornum == 0) {
    ...
    if (DataType == 1 || DataType == 2) {
      ...
      s = "CMLE OR*";
      references[0] =
        'newrow("span6:*Conditional maximum likelihood estimate of Odds Ratio");';
      ORbased[0] =
        'newrow("c:bold:Stratum","c:bold:CMLE OR*","c:bold:span2:' +
        editorschoice1 +
        'Mid-P Limits","c:bold:span2:Fisher Limits");';
      ...
      s = "CMLE RR*";
      references[0] =
        'newrow("span6:*Conditional maximum likelihood estimate of Rate Ratio");';
      //ORbased[0]='newrow("c:bold:Stratum","c:bold:CMLE RR*","c:bold:span2:Mid-P Limits","c:bold:span2:Fisher Limits");'
      //ORbased[0]+='\nline(6)'
      RRbased[0] =
        'newrow("c:bold:Stratum","c:bold:CMLE RR*","c:bold:span2:Mid-P Limits","c:bold:span2:Fisher Limits");';
      RRbased[0] += "\nline(6)";
    }
    ...
    ORbased[index] =
      '\nnewrow("' +
      pstratum +
      '","span2:CMLE Odds Ratio*",' +
      fmtSigFig(cmle, 4) +
      ",";
    ORbased[index] +=
      '"c:span2:' +
      limits(loMidPLim, upMidPLim, 1) +
      '","' +
      editorschoice1 +
      'Mid-P Exact");';
    ORbased[index] +=
      '\nnewrow("","span2:","","c:span2:' +
      limits(loFishLim, upFishLim, 1) +
      '","Fisher Exact");';
    if (DataType == 3) {
      //Person Time data
    ...
    assoc[0] =
      'newrow("c:bold:Stratum","c:span2:bold:Value","c:bold:p-value(1-tail)","c:bold:p-value(2-tail)");';
    assoc[0] += "\nline(5)";
...
function GetCmle(approx) {
  ...
  for (
    i = 0;
    i <= this.degN;
    i++ //  { Defines the numerator polynomial }
  ) {
...
function CalcPoly(DataType) {
  ...
  if (DataType == 1) {
    ...
    alert(
      "DataType of " + DataType + " must be incorrect.  It should be 1,2,or 3"
    );
    return;
  }
    ...
    if (CurTable.informative) {
      ...
      if (DataType == 1) {
        ...
        alert(
          "DataType of " +
            DataType +
            " must be incorrect.  It should be 1,2,or 3"
        );
        return;
      }
...
function PolyStratCC(Table, polyDi) {
  ...
  with (Table) {
    if (informative) {
      ...
      for (i = 1; i <= degDi; i++) {
        polyDi[i] =
          polyDi[i - 1] * ((bb - i) / (aa + i)) * ((cc - i) / (dd + i));

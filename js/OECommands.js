...
function Commandclass() {
  ...
  if (currConfig() == "smartphone") {
    this.htmlheader +=
      "<style>table{align:left;width:80%;}\n img{width:10%; } </style>\n";
...
function TableAsHTML(stratum) {
  ...
  for (i = 0; i < this.data[0].datarmin; i++) {
    if (i < this.data[0].datarmin - 2) {
      this.newrow("<br>");
    } //extra row at beginning, but rarely needed
    if (i == this.data[0].datarmin - 2) {
      //variable label row
      this.newrow();
  ...
  if (
    this.data[0].rowpercents > 0 &&
    this.data[0].rowtots > 0 &&
    this.data[0].coltots > 0
  ) {
...
function CmdIsNumber(content) {
  if (typeof content == "number") {
    return true;
  } else var validFormatRegExp = /^((\+|-)\d)?\d*(\.\d+)?$/;
      ...
      if (strItem.search(/\bimage/i) >= 0) {
        ...
        if (this.mode == "input") {
          this.postcmds[this.postcmds.length] =
            "myTable.setCellColor(" +
            r +
            "," +
            c +
            "," +
            '"' +
            strItem.substring(pos) +
            '")';
        }
      } else {
        //not a known command
...
function InputmyTableCmd(Cmd) {
  if (
    Cmd.search(/\bmoveInput/i) >= 0 ||
    Cmd.search(/\binsert\b/i) >= 0 ||
    Cmd.search(/\bget\b/i) >= 0 ||
    Cmd.search(/\bsetrow\b/i) >= 0
  ) {
...
function InputDataRange(rmin, rmax, cmin, cmax) {
  ...
  if (
    ...
    alert(
      t(
        "Please check that rowmin, rowmax, colmin, and colmax are within the boundaries of the table.\n  The boundaries are: 0,"
      ) +
        this.rows -
        1 +
        t(", 0, and ") +
...
function CmdJavaScript(content) {
  ...
  this.s +=
    "<script language=JavaScript type=text/JavaScript>" + content + "</script>";
}

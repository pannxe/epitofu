...
function storeStratumDataToMem(currStratum, maxAtLeft, maxAtTop, expLeft) {
  ...
  if (rmax == -1 || cmax == -1) {
    ...
    for (tr = rmin; tr <= rmax; tr++) {
      for (tc = cmin; tc <= cmax; tc++) {
        if (
          myTable.row[tr].cell[tc].type == "data" ||
          myTable.row[tr].cell[tc].type == "calendar"
        ) {
  ...
  if (nodata) {
    return false;
  } //Returns false if no data, otherwise true
  else {
...
function readMetaToTable() {
  ...
  if (typeof dataMatrix[0].Dvals != "undefined") {
    copyArray(dataMatrix[0].Dvals, topvals);
  }
  if (typeof dataMatrix[0].Evals != "undefined") {
    copyArray(dataMatrix[0].Evals, leftvals);
  }
...
function writeTable(tableData) {
  ...
  list1 +=
    "Note that they are in a standard format (ExposureLeft with ascending rows and columns),";
  list1 +=
    " but this is only the format for internal storage by OpenEpi.  The statistical module ";
  list1 +=
    "will be able to request the data in any table layout desired, and can, of course, ";
  list1 +=
    "make use of the item names to extract desired elements for processing.  ";
  list1 +=
    "Functions will be provided later for automatic conversion of the data.<br>";
  list1 += "<br>";
  var ix = 0;
  //var val
  var index = new Array();
  ...
  for (stratum = 1; stratum < tableData.length; stratum++) {
    ...
    list =
      "<table><tr><td>&nbsp;<b>Exposure and Disease Status</b></td><td>&nbsp;<b>Value</b></td></tr>";
    ix = 0;
    ...
    s += "</tr></table>";
    list +=
      "</table><h5>The following table has exposure on the left, disease at the top, and (-)(-) upper left.</h5>";

...
(function ($) {
  ...
  function Keypad() {
    ...
    this._defaults = {
      // Global defaults for all the keypad instances
  ...
  $.extend(Keypad.prototype, {
    ...
    _showKeypad: function (field) {
      ...
      if ($.keypad._isDisabledKeypad(field) || $.keypad._lastField == field) {
        // already here
        return;
      }
      ...
      duration =
        duration == "normal" && $.ui && $.ui.version >= "1.8"
          ? "_default"
          : duration;
      var postProcess = function () {
        ...
        inst._mainDiv
          .find("iframe." + $.keypad._coverClass) // IE6- only
          .css({
            left: -borders[0],
            top: -borders[1],
      ...
      if ($.effects && $.effects[showAnim]) {
        inst._mainDiv.show(
          showAnim,
          $.keypad._get(inst, "showOptions"),
          duration,
          postProcess
        );
      } else {
        inst._mainDiv[showAnim || "show"](
          showAnim ? duration : "",
          postProcess
        );
      }
    ...
    _updateKeypad: function (inst) {
      ...
      inst._mainDiv
        .removeClass()
        .addClass(
          this._get(inst, "keypadClass") +
            (this._get(inst, "useThemeRoller")
    ...
    _backValue: function (inst) {
      ...
      if (field.setSelectionRange) {
        // Mozilla
        range =
          inst._input.attr("readonly") || inst._input.attr("disabled")
            ? range
    ...
    _generateHTML: function (inst) {
      ...
      var html = !prompt
        ? ""
        : '<div class="keypad-prompt' +
          (useTR ? " ui-widget-header ui-corner-all" : "") +
          '">' +
          prompt +
          "</div>";
        ...
        for (var j = 0; j < keys.length; j++) {
          ...
          html +=
            keys[j] == this.SPACE
              ...
              : keys[j] == this.HALF_SPACE
              ? '<div class="keypad-half-space"></div>'
              : '<button type="button" class="keypad-key' +
                (useTR ? " ui-state-default" : "") +
                (keys[j] == this.CLEAR
                  ? " keypad-clear" + (useTR ? " ui-state-highlight" : "")
                  : keys[j] == this.BACK
                  ? " keypad-back" + (useTR ? " ui-state-highlight" : "")
                  : keys[j] == this.CLOSE
                  ? " keypad-close" + (useTR ? " ui-state-highlight" : "")
                  : keys[j] == this.SHIFT
                  ? " keypad-shift" + (useTR ? " ui-state-highlight" : "")
                  : keys[j] == this.ENTER
                  ? " keypad-enter" + (useTR ? " ui-state-highlight" : "")
                ...
                (keys[j] == this.CLEAR
                  ...
                  : keys[j] == this.BACK
                  ? this._get(inst, "backStatus")
                  : keys[j] == this.ENTER
                  ? this._get(inst, "enterStatus")
                  : keys[j] == this.CLOSE
                  ? this._get(inst, "closeStatus")
                  : keys[j] == this.SHIFT
                  ? this._get(inst, "shiftStatus")
                  : "") +
                '">' +
                (keys[j] == this.CLEAR
      ...
      html
        ...
        .filter(".keypad-enter")
        .click(function () {
          $.keypad._selectValue(thisInst, $.keypad.EN);
        })
        .end()
        .not(".keypad-clear")
        .not(".keypad-back")
  ...
  $.fn.keypad = function (options) {
    ...
    if (options == "isDisabled") {
      return $.keypad["_" + options + "Keypad"].apply(
        $.keypad,

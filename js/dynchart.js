...
function StratumSubFrom(r, c, val) {
  ...
  if (!this.row[r].cell[c].value) {
    this.row[r].cell[c].value = "0";
  }
  if (isNaN(this.row[r].cell[c].value)) {
    alert("Dynamic Table Error:\nCell value is not an integer");
    return false;
  }
...
function StratumTotalCol(r, c, entry) {
  ...
  for (var i = 0; i < this.rows; i++) //Look for empty cells in column
  {
    if (this.row[i].cell[c].type != null) {
      if (this.row[i].cell[c].type == "data") {
        ...
        if (!isNaN(value)) {
          ...
          if (
            this.emptyrow == -1 &&
            this.row[this.inputR].cell[this.inputC].type != "data"
          ) {
            this.emptyrow = i;
          } else {
            this.emptyrow = -2;
...
function StratumTotalRow(r, c, entry) {
  ...
  this.results = "";
  this.emptycol = -1;
  for (var i = 0; i < this.cols - 1; i++) //Look for empty cells in row
  {

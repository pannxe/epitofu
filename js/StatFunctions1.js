...
function stdRefStr() {
  var refstr;
  refstr =
    '\nnewrow("span6:&deg; &sup1; ' +
    ConfLevel +
    '% confidence limits testing exclusion of 0 or 1, as indicated")';
  var highnote =
    "P-values < " +
    (100 - ConfLevel) / 100 +
    " and confidence limits excluding null values (0,1, or [n]) are highlighted.";
  if (!(Pvalues > 0)) {
...
function fmtSci2Dec(num) {
  ...
  if (efactor > 0) {
    if (finalnum.length > efactor) {
      ...
      finalnum =
        finalnum.substring(0, efactor) + "." + finalnum.substring(efactor);
    } else {
...
function fmtSigFig(num, sig, multby) {
  ...
  if (isNumber(num) && isNumber(multby)) {
    if (num == Number.NEGATIVE_INFINITY || num == Number.POSITIVE_INFINITY) {
      ...
      var factor = Math.pow(
        10,
        sig - Math.ceil(Math.log(Math.abs(num)) / Math.LN10)
      );
...
function limits(lowlim, uplim, markifexclude, multby) {
  ...
  var s =
    fmtSigFig(lowlim, 4, multby) + ",   " + fmtSigFig(uplim, 4, multby) + m;
...
function pchisq(q, df, numonly) {
  ...
  if (q < df) {
    tk =
      (q2 * (1 - nn - df2)) / (df2 + 2 * nn - 1 + (nn * q2) / (df2 + 2 * nn));
    for (kk = nn - 1; kk > 1; kk--)
      tk =
        (q2 * (1 - kk - df2)) /
        (df2 + 2 * kk - 1 + (kk * q2) / (df2 + 2 * kk + tk));
    CFL = 1 - q2 / (df2 + 1 + q2 / (df2 + 2 + tk));
    prob = Math.exp(df2 * Math.log(q2) - q2 - lngamma(df2 + 1) - Math.log(CFL));
  } else {
    tk = (nn - df2) / (q2 + nn);
    for (kk = nn - 1; kk > 1; kk--) tk = (kk - df2) / (q2 + kk / (1 + tk));
    CFU = 1 + (1 - df2) / (q2 + 1 / (1 + tk));
    prob =
      1 -
      Math.exp((df2 - 1) * Math.log(q2) - q2 - lngamma(df2) - Math.log(CFU));
  }
...
function pbeta_raw(x, pin, qin, lower_tail) {
  ...
  if (((p + q) * y) / (p + 1) < eps) {
    ...
    if (xb > lnsml && y != 0) ans = Math.exp(xb);
    if (swap_tail == lower_tail) ans = 1 - ans;
  } else {
    ps = q - Math.floor(q);
    if (ps == 0) ps = 1;
...
function proportion95CL(vx, vN, type) {
  ...
  if (type == "Wilson") {
    ...
    if (vP == 0) var vSL = 0;
      ...
      var vSL =
        ...
        (vP +
          ...
          1.96 *
            Math.sqrt(
              (vx * (vN - vx)) / (vN * vN * vN) + 3.8416 / (4 * vN * vN)
            ));
    pcl[1] = vSL;
    if (vP == 1) var vSU = 1;
    else
    ...
    if (vP == 0) var vSCL = 0;
      ...
      var vSCL =
        (2 * vN * vP +
          ...
          1 -
          1.96 *
            Math.sqrt(3.8416 - (2 + 1 / vN) + 4 * vP * (vN * (1 - vP) + 1))) /
        (2 * (vN + 3.8416));
    pcl[1] = vSCL;
    if (vP == 1) var vSCU = 1;
    else
    ...
    alert(
      "Confidence interval ," +
        type +
        ", does not exist. Legal types are 'Wilson' and 'Fleiss'."
    );
  }
  return pcl;
}

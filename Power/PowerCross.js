...
function CalcPower(data) {
  ...
  powerz =
    (Math.sqrt(n1 * d * d) - z * Math.sqrt((1 + 1 / ratio) * pbar * qbar)) /
    Math.sqrt(p2 * q2 + (p1 * q1) / ratio);
  ...
  powerzcc =
    (Math.sqrt(nn1 * d * d) - z * Math.sqrt((1 + 1 / ratio) * pbar * qbar)) /
    Math.sqrt(p2 * q2 + (p1 * q1) / ratio);
  //alert(powerzcc);
  ...
  with (outTable) {
    ...
    newrow(
      "color#66ffff:span4:bold:l:Two sided-confidence interval (%)",
      "color#ffff99:span4:c:" + fmtSigFig(pt, 6)
    );
    newrow(
      "color#66ffff:span4:bold:l:Number of Exposed",
      "color#ffff99:span4:c:" + fmtSigFig(n1, 6)
    );
    newrow(
      "color#66ffff:span4:bold:l:Prevalence/Coverage among Exposed (%)",
      "color#ffff99:span4:c:" + pp1
    ),
      newrow(
        "color#66ffff:span4:bold:l:Number of Non-exposed",
        "color#ffff99:span4:c:" + fmtSigFig(n2, 6)
      );
    newrow(
      "color#66ffff:span4:bold:l:Prevalence/Coverage among Non-exposed (%)",
      "color#ffff99:span4:c:" + pp2
    );
    newrow(
      "color#66ffff:span4:bold:l:Prevalence/Coverage Ratio",
      "color#ffff99:span4:c:" + fmtSigFig(rr, 2)
    );
    newrow(
      "color#66ffff:span4:bold:l:Prevalence Difference (%)<sup>1",
      "color#ffff99:span4:c:" + dif
    );
    line(8);
    newrow("span4:bold:l:Power based on:: ");
    newrow(
      "span4:l: Normal approximation",
      "span4:c:" + fmtSigFig(powerz, 4) + "%"
    );
    newrow(
      "span4:l: Normal approximation with continuity correction",
      "span4:c:" + fmtSigFig(powerzcc, 4) + "%"
    );
    line(8);
    newrow(
      "span8:l:<sup>1</sup><small> Prevalence Difference = Prevalence in Exposed - Prevalence in Non-exposed."
    );
    endtable();
  }
}

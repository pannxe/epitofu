...
function CalcPower(data) {
  ...
  if (m1 == 0 && m2 == 0 && diff == 0) {
    alert(
      "Missing values! enter mean of group-1 and group-2 (or) enter mean difference of these 2 groups"
    );
  }
  if (diff == 0) {
    diff = Math.abs(m1 - m2);
  ...
  powerz =
    -z +
    (Math.sqrt(n1) * diff) /
      Math.sqrt(Math.pow(s1, 2) + Math.pow(s2, 2) / ratio);
  ...
  with (outTable) {
    ...
    newrow("span7:l: &nbsp;  ");
    newrow(
      "color#66ffff:span7:c:Two-sided " +
        fmtSigFig(pt, 6) +
        "% Confidence Interval"
    );
    ...
    newrow(
      ...
      "color#ffff99:bold:c: Group 1",
      "color#ffff99:r:",
      "color#ffff99:bold:c:Group 2",
      "color#ffff99:",
      "span2:color#ffff99:bold:c:Difference*"
    ); //to insert a new row;
    if (m1 == 0 && m2 == 0) {
      newrow(
    ...
    newrow(
      "color#ffff99::span2:l:Sample size",
      "color#ffff99:span1:c:" + fmtSigFig(n1, 6),
      "color#ffff99:span1:c:",
      "color#ffff99:span1:c:" + fmtSigFig(n2, 6),
    ...
    line(7);
    newrow(
      "span7:l:*<small> Mean difference= (Group 1 mean) - (Group 2 mean)</small>"
    );
    endtable();
  }
}

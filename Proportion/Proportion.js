...
Description +=
  "for simple (binomial) proportions. Entering a numerator and denominator ";
Description +=
  "produces confidence limits calculated by several different methods.";
Description +=
  " The numerator must be smaller than the denominator and both must be positive ";
...
function doStatistics(cmdObj) {
  ...
  function CalcBin() {
    ...
    var msgGTpop = t(
      "Sample denominator must be smaller than population. Please ENTER again."
    );
    if (vN > pop) {
      alert(msgGTpop);
      return;
    ...
    //modified wald;
    var vPP = (vx + zsquare * 0.5) / (vN + zsquare);
    ...
    if (vP == 0) var vSL = 0;
      ...
      var vSL =
        ...
        (vP +
          ...
          zL *
            Math.sqrt(
              (vx * (vN - vx)) / (vN * vN * vN) + zsquare / (4 * vN * vN)
            ));
    if (vP == 1) var vSU = 1;
    else
      var vSU =
    ...
    if (vP == 0) var vSCL = 0;
      ...
      var vSCL =
        (2 * vN * vP +
          ...
          1 -
          zL *
            Math.sqrt(zsquare - (2 + 1 / vN) + 4 * vP * (vN * (1 - vP) + 1))) /
        (2 * (vN + zsquare));
    if (vP == 1) var vSCU = 1;
    else
      var vSCU =
    ...
    var zValue =
      (vx / vN - compareWith) /
      Math.sqrt((compareWith * (1 - compareWith)) / vN);
    var twoSidedP;
    if (zValue >= 0) {
    ...
    with (cmdObj) {
      ...
      if (fpc != null) {
        ...
        newrow(
          "bold:span5:c:Large population size or sample with replacement. "
        );
      }
      newrow(
        "span1:",
        "c:bold:Lower CL",
        "c:bold:Per " + multiplier,
        "c:bold:Upper CL"
      );
      if (multiplier == 100) {
        ...
        newrow(
          "span2:Proportion x " + multiplier,
          fmtSigFig(vP, 6, multiplier)
        );
      }
      ...
      if (vN >= tooBig) {
        ...
        if (T1 != null) {
          newrow(
            "span1:Mid-P Exact",
            fmtSigFig(T1, 4, multiplier),
            "",
            fmtSigFig(T2, 4, multiplier)
          );
        }
        if (DL != null) {
          newrow(
            "span1:Fisher Exact(Clopper-Pearson)",
            fmtSigFig(DL, 4, multiplier),
            "",
            fmtSigFig(DU, 4, multiplier)
          );
        }
      ...
      if (typeof vPL != "undefined") {
        newrow(
          "Wald (Normal Approx.)",
          fmtSigFig(vPL, 4, multiplier),
          "",
          fmtSigFig(vPU, 4, multiplier)
        );
      }
      if (typeof vWL != "undefined") {
        newrow(
          "span1:Modified Wald(Agresti-Coull)",
          fmtSigFig(vWL, 4, multiplier),
          "",
          fmtSigFig(vWU, 4, multiplier)
        );
      }
      if (typeof vSL != "undefined") {
        ...
        newrow(
          "span1:" + editorschoice + "Score(Wilson)*",
          WilsonL,
          "",
          fmtSigFig(vSU, 4, multiplier)
        );
      }
      if (typeof vSL_fpc != "undefined") {
        ...
        newrow(
          "span1:Score with Finite Population Correction",
          WilsonL,
          "",
          fmtSigFig(vSU_fpc, 4, multiplier)
        );
      }
      ...
      if (typeof vSCL != "undefined") {
        ...
        newrow(
          "span1:" + t("Correction (Fleiss Quadratic)"),
          fmtSigFig(vSCL, 4, multiplier),
          "",
          fmtSigFig(vSCU, 4, multiplier)
        );
      }
      ...
      if (typeof zValue != "undefined") {
        newrow(
          "span5:c:One-Sample Test for Binomial Proportion, Normal-Theory Method"
        );
        newrow(
          "span5:c:Does proportion " +
            fmtSigFig(vP, 4) +
            " differ from " +
            fmtSigFig(compareWith, 4) +
            "?"
        );

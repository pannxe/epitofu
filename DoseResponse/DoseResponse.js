...
var Authors =
  "<b>Statistics and Interface</b> <br>" +
...
var Description =
  "Use this module to test for trend " +
...
var Demo =
  "Use the Load Demo Data button to load data from a case-control study of myocardial infarction(MI) in women.  There are five strata representing 5-year age groups from age 25 to 49 with Level 0 being non-smokers, Level 1 smoking 1-24 cigarettes per day, and Level 2 smoking 25 or more per day." +
...
function calcDoseResponse(dataArr) {
  ...
  with (outTable) {
    ...
    p = pchisq(XMHchisq, 1);
    newrow(
      "span6:c:bold:Mantel-Haenszel Summary Odds Ratios and Crude OR for Each Exposure Level"
    );
    newrow(
      "span2:bold:Exposure",
      "span2:bold:MH Summary OR",
      "span2:bold:Crude OR"
    );
    for (i = 0; i < levels; i++) {
      ...
      newrow(
        "span2:Level " + i + " vs. Level 0::",
        "",
        "l:" + fmtFixed(mhsumOR, 3),
        fmtFixed((acrude[i] * dcrude) / (bcrude[i] * ccrude), 3)
      );
      //alert("a=" +acrude[i]+" b="+bcrude[i]+"c="+ccrude+" d="+dcrude)
    }
    newrow();
    newrow(
      "span6:If MH and crude ORs are equal, confounding by the stratifying variable"
    );
    newrow("span6:was not present and stratification is unnecessary.");
    newrow();
    newrow(
      "span5:Extended Mantel-Haenszel chi square for linear trend=",
      "l:" + fmtFixed(XMHchisq, 2)
    );
    newrow("span5:p-value(1 degree of freedom)=", "l:" + p);
    newrow();
    newrow(
      "span6:Includes continuity correction. Rosner,B. Fundamentals of Biostatistics, 5th ed.,Duxbury, 2000, p.606."
    );
    newrow(
      "span6:Results will therefore differ slightly from Statcalc and from Schlesselman,JJ,Case-Control Studies, Oxford, 1982,p.203."
    );
